...
# Create the folder structure (relative to the /lib folder)
mkdir -p ../include macos linux windows
...
mv raylib-${VERSION}_macos/lib/libraylib.500.dylib macos/
cp raylib-${VERSION}_macos/include/raylib.h ../include/

# Extract Linux
tar -xzf raylib-${VERSION}_linux_amd64.tar.gz
mv raylib-${VERSION}_linux_amd64/lib/libraylib.so.500 linux/

# Extract Windows
unzip raylib-${VERSION}_win64_msvc16.zip
mv raylib-${VERSION}_win64_msvc16/lib/raylib.dll windows/
